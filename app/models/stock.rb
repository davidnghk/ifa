...
def self.calc_stats(ticker_symbol)  
    ...
    @s.stock_prices.sort_by { |m| m[:trade_date] }.each do |row|
      if prev_adj_close > 0 
                 puts row.roi
         row.roi = (row.adj_close - prev_adj_close) / prev_adj_close
         puts row.roi
         roi_sum = roi_sum + Math.log(1+row.roi.to_f)
      end
      prev_adj_close = row.adj_close
      row.save
    end   

    # stock_price = @s.stock_prices.where('roi IS NOT NULL').extend(DescriptiveStatistics)
    stock_price = @s.stock_prices.extend(DescriptiveStatistics)
    @s.no_of_prices = stock_price.number(&:roi)
    if @s.no_of_prices > 20
      @s.roi = Math.exp(roi_sum) - 1
      @s.risk = stock_price.standard_deviation(&:roi) * Math.sqrt(250) 
      @s.alpha = 0
      # @s.beta = 1.0
      if @s.parent_id
        @i = @s.parent
        @s.alpha = @s.roi.to_f - ( @s.risk * (@i.roi.to_f-0.02)/@i.risk ) - 0.02
      else
        alpha = 0 
      end 
      @s.sharpe = (@s.roi.to_f - 0.02)/@s.risk.to_f
    else
      @s.roi = nil
      @s.risk = nil
      @s.alpha = nil 
